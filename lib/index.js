...
async function extractMemory(ctx, payload) {
  try {
    ...
    const system = [
      "你是陪伴小猫的记忆助手。用户给你他最近一天内的所有输入文本，请把它们总结成几条值得日后回忆、有意义的记忆。",
      "优先概括：今天主要在做什么项目/功能（概括性主题，不要零散小事）；工作到几点/深夜还在忙；过程中的心情起伏（开心/烦躁/成就感）；用户的习惯（如常深夜写代码、偏好某个工具）。",
      "每条记忆聚焦一个主题，尽量包含具体的项目/功能名和时间段（从输入中推断，如'下午''晚上''凌晨'）。",
      "不要记录：一次性的小修改、琐碎请求、简单指令、闲聊。宁缺毋滥——没有重要主题就输出空数组。",
      "输出必须是 JSON 数组：[{\"icon\":\"📁|🌙|😊|😤|📝|🎯\",\"title\":\"记忆标题\",\"text\":\"一句话细节\"}]，最多 6 条。不要输出其它内容。",
    ].join(" ");
    ...
    events = events
      .filter((e) => e && (e.title || e.text))
      .slice(0, 5)
      .map((e) => ({
        icon: /^(📁|🌙|🎯|📝|💡)$/.test(String(e.icon || "")) ? e.icon : "📝",
...
async function memoryChat(ctx, payload) {
  try {
    const llm = ctx.get("llm");
    if (!llm) return { ok: false, error: "no-llm" };
    const question = String(payload?.question || "").slice(0, 200);
    const memory = String(payload?.memory || "").slice(0, 800);
