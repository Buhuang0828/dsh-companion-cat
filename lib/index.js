...
/**
 * Sum the token-meter's current surface across all live sessions. Falls back
 * gracefully when the services are absent, so the plugin never breaks.
 */
async function fetchTotalTokens(ctx) {
  try {
    if (!ctx.sessions || !ctx.tokenMeter) return { ok: false, error: "unavailable" };
    /* Cordis services are lazy: wait for readiness before touching them */
    if (ctx.sessions.ready) await ctx.sessions.ready;
    if (ctx.tokenMeter.ready) await ctx.tokenMeter.ready;
    let total = 0;
    for (const session of ctx.sessions.list()) {
      try {
        const m = ctx.tokenMeter.measure(session);
        if (m && typeof m.totalTokens === "number") total += m.totalTokens;
      } catch {
...
async function generateHint(ctx, profile) {
  try {
    if (!ctx.llm) return { ok: false, error: "no-llm" };
    if (ctx.llm.ready) await ctx.llm.ready;
