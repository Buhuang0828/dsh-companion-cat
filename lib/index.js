...
async function extractMemory(ctx, payload) {
  try {
    ...
    const system = [
      "你是记忆提炼助手。用户给你一些他最近输入的文本（可能是工作内容、计划、项目、心情等）。",
      "从中提炼值得记住的重要事件：项目/任务进展、正在做的目标、深夜工作、重要计划、用户特别在意的事。",
      "忽略闲聊和无关内容。每条事件一句话，不超过 30 字，有温度、口语化。",
      "输出必须是 JSON 数组：[{\"icon\":\"📁|🌙|🎯|📝|💡\",\"title\":\"事件标题\",\"text\":\"一句话细节\"}]，最多 5 条。不要输出其它内容。",
    ].join(" ");
    const userText =
      "用户最近的输入：\n" +
