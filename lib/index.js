...
async function extractMemory(ctx, payload) {
  try {
    ...
    events = events
      .filter((e) => e && String((e.text || e.title) || "").trim())
      .slice(0, 4)
      .map((e) => ({
...
export {
  apply, inject, name,
  gateMemoryQuestion, buildHintPrompt, parseHintReply, makeUserMessage,
  sessionUsage, usageTotal, fetchTotalTokens, fetchSessionsRanking,
  fetchBalance, extractMemory, memoryChat, generateHint,
};
