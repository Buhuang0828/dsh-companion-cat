...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      /* storybook button right below the toolbar, same frosted style */
      "#companion-pet-memory-btn{display:flex;align-items:center;gap:6px;padding:6px 14px;border-radius:999px;border:1px solid rgba(194,178,128,.4);background:rgba(250,240,215,.5);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);color:#8a7448;font-size:12px;font-weight:700;letter-spacing:.04em;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.22);transition:background .15s,transform .15s}" +
      "#companion-pet-memory-btn:hover{background:rgba(240,226,190,.65);transform:translateY(-1px)}" +
      "#companion-pet-memory-btn.off{opacity:.45;cursor:not-allowed}" +
      "#companion-pet-memory-btn.off:hover{background:rgba(250,240,215,.5);transform:none}" +
      ...
      ".pet-memory-panel .mp-extract{border:1px solid rgba(194,178,128,.5);background:linear-gradient(180deg,#f2c462,#d9a33a);color:#fff;border-radius:999px;padding:7px 14px;font-size:12px;font-weight:700;cursor:pointer;box-shadow:0 2px 7px rgba(180,130,50,.35);transition:filter .15s}" +
      ".pet-memory-panel .mp-extract:hover{filter:brightness(1.07)}" +
      ".pet-memory-panel .mp-extract:disabled{opacity:.45;cursor:not-allowed;filter:none}" +
    ...
    function createPet() {
      ...
      function openMemoryPanel() {
        ...
        if (m) {
          ...
          items.push({
            ...
            title: "第一次相遇",
            text:
              "那天你挑选了很久，最后把我选中了。从那天起，我就陪你一起工作啦。",
          });
          ...
          (m.notes || [])
            ...
            .reverse()
            .forEach(function (n) {
              items.push({ date: n.date, icon: n.icon || "📝", title: n.title || "记忆", text: n.text });
            });
        ...
        var listHtml =
          items.length === 0
            ? '<div class="mp-empty">还没有故事……选一只小猫，故事就开始了。</div>'
            : items
                .map(function (it) {
                  return (
                ...
                .join("");
        var catKey = readConfig().cat;
        var catNow = CATS[catKey] || CATS.orange;
        memPanel = document.createElement("div");
        memPanel.className = "pet-memory-panel";
        memPanel.innerHTML =
          '<div class="mp-title-bar">📖 我们的时光<button type="button" class="mp-close" title="关闭">✕</button></div>' +
          '<div class="mp-list">' +
          listHtml +
          "</div>" +
          '<div class="mp-ask"><div class="mp-ask-title">🧠 记忆提炼</div>' +
          '<button type="button" class="mp-extract">✨ 提炼最近的输入为记忆</button>' +
          '<div class="mp-ask-title">💌 问问' +
          catNow.name +
          "</div>" +
        ...
        var qs = memPanel.querySelectorAll(".mp-q");
        qs.forEach(function (btn) {
          btn.addEventListener("click", function () {
            askMemoryChat(btn.dataset.q);
          });
        });
        var extractBtn = memPanel.querySelector(".mp-extract");
        if (extractBtn) {
          extractBtn.addEventListener("click", function () {
            extractMemoryNow();
          });
        }
        /* asking questions & extraction cost tokens: gate behind 深度陪伴 */
        ...
        if (!readConfig().deepCompanion) {
          ...
          if (answerEl) {
            answerEl.textContent = "打开设置里的「深度陪伴」后，就可以让我记住更多、回答你啦~";
          }
        }
      ...
      function askMemoryChat(question) {
        var answerEl = memPanel && memPanel.querySelector(".mp-answer");
        if (!answerEl) return;
      ...
      function extractMemoryNow() {
        var inputs = collectUserInputs();
        if (!inputs.length) {
          say("没有新的、值得记住的输入可以提炼啦~");
          return;
        }
      ...
      var memBtn = null; /* storybook button, anchored below the toolbar */
      var lastMemBtnLeft = -1;
      function anchorToolbar() {
        try {
          ...
          /* storybook button: horizontally centered under the mode group */
          if (memBtn) {
            var modes = toolbar.querySelector(".pt-modes");
            if (modes) {
              var mr = modes.getBoundingClientRect();
              var mw = memBtn.offsetWidth || 44;
              var mc = Math.max(8, Math.round(mr.left + mr.width / 2 - mw / 2));
              if (mc !== lastMemBtnLeft) {
                lastMemBtnLeft = mc;
                memBtn.style.left = mc + "px";
              }
            } else {
              memBtn.style.left = target + "px";
            }
          }
        } catch (_) {}
      }
      ...
      function showToolbar() {
        ...
        if (!memBtn) {
          ...
          memBtn.type = "button";
          memBtn.id = "companion-pet-memory-btn";
          memBtn.title = "我们的时光";
          memBtn.textContent = "📖";
          memBtn.style.position = "fixed";
          memBtn.style.bottom = "112px";
          memBtn.style.left = "320px";
          memBtn.style.zIndex = "2147483001";
          document.body.appendChild(memBtn);
          memBtn.addEventListener("click", function () {
            openMemoryPanel();
          });
        }
        /* dim when 深度陪伴 is off (asking questions costs tokens) */
        memBtn.classList.toggle("off", !readConfig().deepCompanion);
        anchorToolbar();
        startToolbarWatch();
      }
      ...
      return function dispose() {
        ...
        closeStatsPanel();
        closeMemoryPanel();
        if (memBtn && memBtn.parentNode) memBtn.parentNode.removeChild(memBtn);
        if (panel) {
          panel.remove();
          panel = null;
        }
