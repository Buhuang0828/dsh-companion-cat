...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var KEYS = {
      ...
      lastFp: "dsh-companion-cat:last-fp",
    ...
    function createPet() {
      ...
      function updateLastBaseline() {
        try {
          ...
          localStorage.setItem(KEYS.lastFp, JSON.stringify(fp.slice(-200)));
        } catch (_) {}
      }
        ...
        fetch("/companion-pet/api/extract-memory", {
          ...
          .then(function (d) {
            if (d && d.ok && Array.isArray(d.events) && d.events.length) {
              ...
              localStorage.setItem(KEYS.autoExtract, target);
      ...
      function maybeAutoExtract() {
        try {
          ...
          /* find the OLDEST day (before today) with a bucket that hasn't
             been compressed yet */
          var map = readInputsByDay();
          var done = localStorage.getItem(KEYS.autoExtract) || "";
