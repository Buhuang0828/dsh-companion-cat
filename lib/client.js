...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var BACKGROUNDS = {
      mushroom: {
        name: "蘑菇屋",
        day: ASSET_BASE + "/background-day.png",
        night: ASSET_BASE + "/background-night.png",
        live: ASSET_BASE + "/background-live.mp4",
      },
      cathouse: {
        name: "猫猫屋",
     ...
     */
    var CATS = {
      /* display order (left → right in the carousel): fold, cat7, gray,
         orange, white, dark, cat8, black — orange & white sit at the
         center, gray & dark beside them, cat7 & cat8 next, fold & black at
         the outer ends */
      fold: {
        name: "折折",
        ...
        persona: "憨憨的，有点笨拙可爱",
        scale: 1.22,
        /* 折折 has happy / sad / scare + idle (per its gif folder) */
        acts: ["happy", "sad", "scare"],
      },
      cat7: {
        name: "绵绵",
        dir: ASSET_BASE + "/cats/cat7",
    ...
    var CSS =
      ...
      "#companion-pet-mem-bar .pt-cap{border:1px solid rgba(194,178,128,.4);background:rgba(250,240,215,.5);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);color:#8a7448;font-size:12px;font-weight:700;letter-spacing:.04em;cursor:pointer;border-radius:999px;padding:6px 12px;box-shadow:0 4px 12px rgba(0,0,0,.22);transition:background .15s,transform .15s}" +
      ...
      /* compact daily memory rows (envelope view) */
      ".pet-memory-panel .np-list{margin-top:4px;display:flex;flex-direction:column;gap:10px}" +
      ".pet-memory-panel .np-day{display:flex;flex-direction:column}" +
      ".pet-memory-panel .np-day-title{font-size:11px;font-weight:700;color:#a08040;letter-spacing:.05em;margin:2px 0 4px}" +
      ".pet-memory-panel .np-row{display:flex;gap:8px;align-items:baseline;font-size:12px;padding:3px 2px;border-bottom:1px dashed rgba(120,100,60,.12)}" +
      ".pet-memory-panel .np-row .np-icon{flex:none}" +
      ".pet-memory-panel .np-row .np-title{font-weight:600;color:#5a4632;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:38%}" +
      ".pet-memory-panel .np-row .np-text{color:#8a7448;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
      /* ask-the-cat section inside the storybook */
      ".pet-memory-panel .mp-ask{margin-top:14px;padding-top:12px;border-top:1px dashed rgba(120,100,60,.25);display:flex;flex-direction:column;gap:6px}" +
      ".pet-memory-panel .mp-ask-title{font-size:13px;font-weight:800;color:#a08040;margin-bottom:2px}" +
      ".pet-memory-panel .mp-q{border:1px solid rgba(194,178,128,.45);background:rgba(255,253,246,.7);color:#6a5538;border-radius:999px;padding:6px 12px;font-size:12px;text-align:left;cursor:pointer;transition:background .15s}" +
      ".pet-memory-panel .mp-q:hover{background:rgba(226,196,120,.25)}" +
      ".pet-memory-panel .mp-q:disabled{opacity:.45;cursor:not-allowed}" +
      ".pet-memory-panel .mp-q:disabled:hover{background:rgba(255,253,246,.7)}" +
      ".pet-memory-panel .mp-answer{margin-top:4px;font-size:12.5px;color:#5a4632;background:rgba(252,243,230,.9);border:1px solid rgba(194,178,128,.4);border-radius:12px;padding:10px 12px;min-height:20px;line-height:1.6}" +
      ".pet-memory-panel .mp-answer.loading{color:#a08a60;font-style:italic}" +
    ...
    function syncBgVideo(useLive, veil) {
      if (useLive) {
        ensureBgVideo();
        /* keep the video synced to the active background set */
        var set = BACKGROUNDS[readConfig().bgSet] || BACKGROUNDS.mushroom;
        if (set.live && bgVideo.src.indexOf(set.live) < 0) {
          bgVideo.src = set.live;
        }
    ...
    function applyWallpaper() {
      ...
      /* auto mode => live video backdrop (only sets that ship a video);
         day/night => static image */
      var set = BACKGROUNDS[cfg.bgSet] || BACKGROUNDS.mushroom;
      var useLive = cfg.mode === "auto" && !!set.live;
    ...
    function createPet() {
       ...
       */
      var BAL_SNAP_KEY = "dsh-companion-cat:bal-snap";
      function recordBalanceCost(total) {
        try {
          var today = todayStr();
          var snap = JSON.parse(localStorage.getItem(BAL_SNAP_KEY) || "null");
          ...
          localStorage.setItem(BAL_SNAP_KEY, JSON.stringify({ date: today, total: total }));
        } catch (_) {}
      }
      ...
      function tickStats() {
        try {
          ...
          if (startDay !== today && startDay) {
            ...
            var nightSec = Math.max(0, Math.floor((dayStart.getTime() - prev) / 1000));
            if (nightSec > 0) {
              d[startDay] = d[startDay] || { s: 0, t: 0, k: 0 };
              d[startDay].s += nightSec;
            }
            var daySec = Math.max(0, Math.floor((now - dayStart.getTime()) / 1000));
      ...
      function resetTokenHistory() {
        try {
          var key = "dsh-companion-cat:daily-token-v1";
      ...
      function resetToday() {
        try {
          var key = "dsh-companion-cat:daily-reset-v2";
          if (localStorage.getItem(key)) return;
          localStorage.setItem(key, "1");
          var d = readDaily();
          var t = todayStr();
      ...
      function buildTimelineRows(m) {
        ...
        if (m) {
          ...
          (m.sessions || [])
            ...
            .reverse()
            .forEach(function (s) {
              items.push({ date: s.date, icon: "📁", title: "工作记录", text: s.title });
            });
      ...
      function openMemoryPanel() {
        ...
        memPanel.innerHTML =
          ...
          '<button type="button" class="mp-notes-btn" title="查看记忆">✉️ 记忆</button>' +
      ...
      /* ---- ✉️ memory envelope: extraction + ask-the-cat + daily memory ---- */
      var notesPanel = null;
      ...
      function npRowHtml(n) {
        return (
          '<div class="np-row"><span class="np-icon">' +
          (n.icon || "📝") +
          '</span><span class="np-title" title="' +
          n.title +
          '">' +
          (n.title || "") +
          '</span><span class="np-text" title="' +
          n.text +
          '">' +
          (n.text || "") +
          "</span></div>"
      ...
      function openNotesPanel() {
        ...
        /* group memories by day, compact rows */
        var groups = {};
        ...
        (m ? m.sessions || [] : []).forEach(function (s) {
          (groups[s.date] = groups[s.date] || []).push({
            icon: "📁",
            title: "工作记录",
            text: s.title,
          });
        ...
        var listHtml =
          dates.length === 0
            ? '<div class="mp-empty">还没有记忆，点上面的「✨ 提炼」开始记录。</div>'
            : dates
                .map(function (d) {
                  return (
                    ...
                    d +
                    "</div>" +
                    groups[d].map(npRowHtml).join("") +
                    "</div>"
                  );
                })
                .join("");
        notesPanel = document.createElement("div");
        notesPanel.className = "pet-memory-panel";
        notesPanel.innerHTML =
          '<div class="mp-title-bar"><span>✉️ 猫猫的记忆</span><button type="button" class="mp-close" title="关闭">✕</button></div>' +
        ...
        notesPanel.querySelectorAll(".mp-q").forEach(function (btn) {
          btn.addEventListener("click", function () {
            askMemoryChat(btn.dataset.q);
          });
        });
        var answerEl = notesPanel.querySelector(".mp-answer");
      ...
      function askMemoryChat(question) {
        ...
        fetch("/companion-pet/api/memory-chat", {
          ...
          .then(function (d) {
            ...
            if (d && d.ok && d.text) {
              answerEl.textContent = "🐾 " + d.text;
            } else if (d && d.error === "budget") {
              answerEl.textContent = "它今天说了太多话，明天再问它吧~";
            } else {
          ...
          .catch(function () {
            answerEl.classList.remove("loading");
            answerEl.textContent = "它有点走神了，再试一次吧~";
          });
      }
      ...
      var MEM_KEY = "dsh-companion-cat:memory";
      function readMem() {
        try {
          var m = JSON.parse(localStorage.getItem(MEM_KEY) || "null");
       ...
       */
      var EXTRACTED_KEY = "dsh-companion-cat:extracted";
      var LAST_FP_KEY = "dsh-companion-cat:last-fp";
      var LAST_START_KEY = "dsh-companion-cat:last-session-start";
      var lastSessionStart = 0; /* when the PREVIOUS session began */
      /**
       * Baseline: the fingerprints of messages already on screen at load.
       * They belong to EARLIER sessions; the next open summarizes only what
       * appeared AFTER this baseline (this session's new input).
       */
      function updateLastBaseline() {
        try {
          ...
          document.querySelectorAll('[class*="userRow"]').forEach(function (r) {
            ...
            if (!t) return;
            fp.push(t.length + ":" + t.slice(0, 40));
          });
          localStorage.setItem(LAST_FP_KEY, JSON.stringify(fp.slice(-200)));
        } catch (_) {}
      }
      ...
      function collectUserInputs() {
        /* skip fingerprints from the LAST load (earlier sessions) and any
           already extracted — only THIS session's new input is collected */
        var skip = {};
        try {
          JSON.parse(localStorage.getItem(LAST_FP_KEY) || "[]").forEach(function (h) {
            skip[h] = true;
          });
        } catch (_) {}
        var seen = [];
        try {
          seen = JSON.parse(localStorage.getItem(EXTRACTED_KEY) || "[]");
        } catch (_) {}
        ...
        try {
          localStorage.setItem(EXTRACTED_KEY, JSON.stringify(seen.slice(-100)));
        } catch (_) {}
        return out;
      }
      function extractMemoryNow(silent) {
        var inputs = collectUserInputs();
        /* only summarize once enough new input has piled up */
        if (inputs.length < 5) {
          if (!silent) say("聊得还不够多，再多聊几句我再帮你记~");
          return;
        }
        var timeSpan = "";
        if (lastSessionStart) {
          var st = new Date(lastSessionStart);
          var en = new Date();
          timeSpan =
            "（该会话大致时段：从 " +
            (st.getMonth() + 1) +
        ...
        fetch("/companion-pet/api/extract-memory", {
          ...
          .then(function (d) {
            if (d && d.ok && Array.isArray(d.events) && d.events.length) {
              ...
              m.notes = m.notes.slice(-30);
              writeMem(m);
              localStorage.setItem(
                AUTO_EXTRACT_KEY,
                lastSessionStart ? dateStrOf(lastSessionStart) : today
              );
              say(
                "我都记下来啦：" +
                  d.events
      ...
      /**
       * Automatic extraction (next-open summary): with 深度陪伴 on, on each
       * page load we summarize the PREVIOUS session's new input — once per
       * previous-session date, so it never fires repeatedly within a day.
       */
      var AUTO_EXTRACT_KEY = "dsh-companion-cat:auto-extract";
      function maybeAutoExtract() {
        try {
          if (!readConfig().deepCompanion) return;
          var lastDate = lastSessionStart ? dateStrOf(lastSessionStart) : todayStr();
          if (localStorage.getItem(AUTO_EXTRACT_KEY) === lastDate) return;
          setTimeout(function () {
            extractMemoryNow(true);
          }, 12000);
        } catch (_) {}
      }
      ...
      function initMemory() {
        try {
          ...
          lastSessionStart = parseInt(localStorage.getItem(LAST_START_KEY) || "0", 10) || 0;
          localStorage.setItem(LAST_START_KEY, String(Date.now()));
          setTimeout(updateLastBaseline, 5000);
          var today = todayStr();
      ...
      function anchorToolbar() {
        try {
          ...
          /* mem buttons: horizontally centered under the mode group */
          if (memBar) {
            var modes = toolbar.querySelector(".pt-modes");
            if (modes) {
              var mr = modes.getBoundingClientRect();
              var bw = memBar.offsetWidth || 120;
              var bc = Math.max(8, Math.round(mr.left + mr.width / 2 - bw / 2));
      ...
      function showToolbar() {
        ...
        if (!memBar) {
          ...
          memBar.innerHTML =
            '<button type="button" class="pt-cap" id="companion-pet-memory-btn">📖 时光</button>';
