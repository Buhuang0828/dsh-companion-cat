...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPet() {
      ...
      /* ---- approval reminders: DSH asks "allow this operation?" — watch
         the [data-approval-key] panel appear, bubble + kitty + system
         notification so it is never missed even in another window ---- */
      var approvalSeen = new Set();
      var approvalPermAsked = false;
      function notifyApproval(key) {
        if (!key || approvalSeen.has(key)) return;
        approvalSeen.add(key);
        say("⚠️ 有个操作等你批准！");
        transientAction("think");
        try {
          if (!("Notification" in window)) return;
          if (Notification.permission === "granted") {
            var n = new Notification("🐱 陪伴小猫", {
              body: "有个操作等你批准，点我处理~",
              tag: "pet-approval",
            });
            n.onclick = function () {
          ...
          } else if (Notification.permission !== "denied" && !approvalPermAsked) {
            approvalPermAsked = true;
            Notification.requestPermission().then(function (p) {
              if (p === "granted") {
                var n2 = new Notification("🐱 陪伴小猫", {
                  body: "有个操作等你批准，点我处理~",
                  tag: "pet-approval",
                });
                n2.onclick = function () {
      ...
      function watchApprovals() {
        try {
          /* already-open approvals on page load */
          document.querySelectorAll("[data-approval-key]").forEach(function (el) {
            notifyApproval(el.getAttribute("data-approval-key"));
          });
          var MO = window.MutationObserver;
          if (!MO) return;
          var obs = new MO(function (muts) {
            if (!readConfig().approvalRemind) return;
            for (var i = 0; i < muts.length; i++) {
              var nodes = muts[i].addedNodes;
              for (var j = 0; j < nodes.length; j++) {
                var n = nodes[j];
                if (!n || n.nodeType !== 1) continue;
                var el = n.matches && n.matches("[data-approval-key]")
                  ? n
                  : n.querySelector
                    ? n.querySelector("[data-approval-key]")
                    : null;
                if (el) notifyApproval(el.getAttribute("data-approval-key"));
              }
            }
          });
          obs.observe(document.body, { childList: true, subtree: true });
          /* safety poll: some approval UIs mount outside the observed subtree
             or before the observer attaches — check every 10s anyway (the
             key set dedupes, so this never double-alerts) */
          timers.push(
      ...
      function openPanel() {
        ...
        panel.innerHTML =
          ...
          row("审批提醒", "onoff", "approvalRemind", cfg.approvalRemind) +
        ...
        panel.querySelectorAll("[data-key]").forEach(function (el) {
          el.addEventListener("change", function () {
            ...
            if (key === "bg" || key === "bubbles" || key === "lateRemind" || key === "mood" || key === "restRemind" || key === "approvalRemind" || key === "smartCompanion") {
              cfg[key] = el.checked;
