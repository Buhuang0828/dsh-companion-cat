...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPet() {
      ...
      /** Count newly added user message rows (no backtrack, no duplicates). */
      function watchTurns() {
        try {
          ...
          var obs = new MO(function (muts) {
            ...
            for (var i = 0; i < muts.length; i++) {
              ...
              for (var j = 0; j < nodes.length; j++) {
                ...
                if (n.matches && n.matches('[class*="userRow"]') && !countedRows.has(n)) {
                  countedRows.add(n);
                  added++;
                }
                if (n.querySelectorAll) {
                  ...
                  for (var k = 0; k < rows.length; k++) {
                    if (!countedRows.has(rows[k])) {
                      countedRows.add(rows[k]);
                      added++;
                    }
                  }
                }
