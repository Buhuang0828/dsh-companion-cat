...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      /* kraft-paper journal — the WHOLE panel is one sheet of warm brown
         paper: soft grain via an inline SVG turbulence, gentle paper-tone
         gradient, deckle-ish edge. No ruled lines, no icons — plain and
         quiet like a real notebook page. */
      ".pet-memory-panel.journal{background:" +
        "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='160' height='160'><filter id='g'><feTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='2' stitchTiles='stitch'/><feColorMatrix type='saturate' values='0'/><feComponentTransfer><feFuncA type='linear' slope='0.05'/></feComponentTransfer></filter><rect width='100%' height='100%' filter='url(%23g)'/></svg>\"),linear-gradient(165deg,#f6e8c8 0%,#efdcb2 45%,#e5cd9c 100%)!important;" +
        "width:min(94vw,560px)!important;" +
        "border:1px solid rgba(150,110,55,.45)!important;" +
        "box-shadow:inset 0 1px 0 rgba(255,255,255,.45),inset 0 0 42px rgba(140,95,40,.14),0 20px 55px rgba(60,40,15,.42)!important;" +
        "border-radius:26px 14px 30px 12px / 14px 28px 12px 32px!important;color:#4c381f!important}" +
      ".pet-memory-panel.journal .mp-title-bar{color:#6b4a22;font-weight:800;border-bottom:1px dashed rgba(140,95,40,.35);padding-bottom:10px}" +
      ".pet-memory-panel.journal .mp-close{background:rgba(140,95,40,.16);color:#6b4a22}" +
      ".pet-memory-panel.journal .mp-close:hover{background:rgba(190,80,55,.3);color:#fff}" +
      ".pet-memory-panel.journal .np-list{margin:6px 0 0;display:flex;flex-direction:column;gap:16px;padding:4px 2px}" +
      ".pet-memory-panel.journal .np-day{display:flex;flex-direction:column}" +
      ".pet-memory-panel.journal .np-day-title{font-size:13px;font-weight:700;color:#7a4a20;letter-spacing:.05em;margin:0 0 4px;display:flex;align-items:baseline;gap:8px}" +
      ...
      ".pet-memory-panel.journal .np-row{display:flex;gap:8px;font-size:13px;line-height:1.6;padding:3px 2px;color:#4c381f}" +
      ".pet-memory-panel.journal .np-row .np-num{flex:none;min-width:22px;text-align:right;color:#a0703a;font-weight:700;font-size:12.5px}" +
      ".pet-memory-panel.journal .np-row .np-body{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
      ".pet-memory-panel.journal .np-row:nth-child(odd){background:rgba(120,90,40,.05);border-radius:8px;padding:3px 6px}" +
      ...
      ".pet-memory-panel .mp-answer{margin-top:6px;font-size:12.5px;color:#5a4526;background:rgba(243,232,205,.75);border:1px solid rgba(180,150,90,.45);border-radius:12px;padding:10px 13px;min-height:20px;line-height:1.65}" +
    ...
    function createPet() {
      ...
      function openNotesPanel() {
        ...
        notesPanel.innerHTML =
          ...
          /* memory list FIRST: a kraft-paper journal of every recorded day */
          '<div class="np-list">' +
          listHtml +
          "</div>" +
          /* ask-the-cat BELOW the journal */
      ...
      var MEM_CLEAN_KEY = "dsh-companion-cat:mem-clean-v3";
      function cleanTrivialMemories() {
        try {
          if (localStorage.getItem(MEM_CLEAN_KEY)) return;
          var m = readMem();
          ...
          if (m.notes.length !== before) {
            writeMem(m);
            localStorage.setItem(MEM_CLEAN_KEY, "1");
          }
        } catch (_) {}
      }
      function initMemory() {
