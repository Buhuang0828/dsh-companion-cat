  ...
  factory: (require) => {
    ...
    var CATS = {
      ...
      fold: {
        ...
        /* nuzzle/lick/happy 内容比 idle 略高——缩回 idle 视觉大小。
           开心跳跳跃最高点会略超出画布顶（GIF 内容特性，素材侧限制），
           保持位置不动，接受跳跃瞬间一点点裁切。 */
        actScale: { nuzzle: 0.915, lick: 0.824, happy: 0.964 },
      },
      mianmian: {
        name: "绵绵",
      ...
      gray: {
        ...
        hasClick: true,
        /* stretch draws extra tall (bbox 304 vs 260) — shrink it to match */
        actScale: { stretch: 0.855 },
      },
      orange: {
        name: "橘橘",
      ...
      dark: {
        ...
        /* scare/sleep/chase draw small (bbox 232-238 vs 270) — bump them */
        actScale: { scare: 1.164, sleep: 1.134, chase: 1.144 },
      },
      tiaotiao: {
        name: "跳跳",
        ...
        /* lick/nuzzle draw slightly smaller than idle (bbox 178/174 vs 182) */
        actScale: { nuzzle: 1.046, lick: 1.022 },
      },
      black: {
        name: "墨墨",
        ...
        /* happy/walk/celebrate draw small (bbox 216-222 vs 260) — bump them;
           click/sad/sleep/stretch 略小 — 微调 */
        actScale: {
          happy: 1.182, walk: 1.204, celebrate: 1.171,
          click: 1.016, sad: 1.024, sleep: 1.008, stretch: 1.008,
        },
      },
    };
     ...
     */
    /* Real frame-clock length (ms) of each shipped gif — measured from the
       GIF graphic-control extensions (static assets, so hard-coded once).
       Transient actions run exactly one full loop + a little slack, so a
       skill never gets cut short mid-motion or left hanging on a stale
       frame. Key: "<cat>:<action>". */
    /* Real frame-clock length (ms) of each shipped gif — measured via the
       same GIF decoder the browser uses (System.Drawing per-frame delays).
       Almost every action is a standard 81-frame loop = 8100ms, so the
       default below covers the bulk; only non-standard lengths are listed.
       Transient actions run exactly one full loop + a little slack, so a
       skill never gets cut short mid-motion or left hanging on a stale
       frame. Key: "<cat>:<action>". */
    var GIF_DUR_MS = {
      /* fold (fold gifs run 81-100 frames) */
      "fold:happy": 8700, "fold:scare": 8200, "fold:sleep": 10000,
      /* dark (dark gifs vary) */
      "dark:chase": 10800, "dark:happy": 8000, "dark:scare": 7600,
      "dark:sleep": 9500, "dark:stretch": 7700,
      /* tiaotiao (shorter loops) */
      "tiaotiao:happy": 7500, "tiaotiao:lick": 6300, "tiaotiao:nuzzle": 5200,
      "tiaotiao:stretch": 6600,
    };
    /** Default transient duration: the standard 81-frame loop. */
    var GIF_DUR_DEFAULT = 8100;
    function actionsFor(cat) {
      var dir = cat.dir;
      /* default transient duration: enough for any shipped gif to complete
         one full loop; overridden per action from the file's real length */
      var def = { url: dir + "/idle.gif", h: 203, dur: GIF_DUR_DEFAULT + 600, aScale: 1 };
      var out = { idle: def };
      if (cat.hasClick) {
        var ck = catKeyOf(cat) + ":click";
        out.click = { url: dir + "/click.gif", h: 269, dur: (GIF_DUR_MS[ck] || GIF_DUR_DEFAULT) + 600, aScale: 1 };
      }
      ...
      (cat.acts || []).forEach(function (k) {
        ...
        if (sp) {
          ...
          var dur = (GIF_DUR_MS[catKeyOf(cat) + ":" + k] || GIF_DUR_DEFAULT) + 600;
          out[k] = { url: dir + "/" + sp[0], h: sp[1], dur: dur, aScale: aScale };
        }
      });
      return out;
    }
    /** Reverse-lookup a cat's key from its object (GIF duration table key). */
    function catKeyOf(cat) {
      var keys = Object.keys(CATS);
      for (var i = 0; i < keys.length; i++) {
        if (CATS[keys[i]] === cat) return keys[i];
      }
      return "fold";
    }
    ...
    function createPet() {
      ...
      function playAction(name) {
        ...
        /* replay the same action if asked again (e.g. double-click or an
           ambient pick equal to the current one): swap the src through a
           cache-buster so the gif restarts from frame 0 instead of the
           "same action" early-return that froze it mid-loop */
        if (currentAction === name && img.src.indexOf(a.url) >= 0) {
          img.src = a.url + (a.url.indexOf("?") >= 0 ? "&" : "?") + "r=" + Date.now();
          return;
        }
        currentAction = name;
        img.src = a.url;
