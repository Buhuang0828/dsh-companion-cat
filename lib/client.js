...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPet() {
      ...
      var INPUTS_BY_DAY_KEY = "dsh-companion-cat:inputs-by-day";
      var lastSessionStart = 0; /* when the PREVIOUS session began */
      function readInputsByDay() {
        try {
          return JSON.parse(localStorage.getItem(INPUTS_BY_DAY_KEY) || "{}") || {};
        } catch (_) {
          return {};
        }
      }
      function writeInputsByDay(map) {
        try {
          localStorage.setItem(INPUTS_BY_DAY_KEY, JSON.stringify(map));
        } catch (_) {}
      }
      /**
      ...
      function collectUserInputs() {
        ...
        try {
          localStorage.setItem(EXTRACTED_KEY, JSON.stringify(seen.slice(-500)));
        } catch (_) {}
        if (out.length) {
      ...
      /**
       * Compress ONE whole day's inputs into memories. dayStr is the date
       * whose bucket gets summarized; the resulting notes are dated that
       * day. Falls back to collecting today's live inputs when no bucket
       * exists (first-run path).
       */
      function extractMemoryNow(dayStr, silent) {
        var map = readInputsByDay();
        var target = dayStr || todayStr();
        var inputs = (map[target] || []).slice(0, 60);
        /* only summarize once enough input has piled up */
        if (inputs.length < 5) {
        ...
        fetch("/companion-pet/api/extract-memory", {
          ...
          .then(function (d) {
            if (d && d.ok && Array.isArray(d.events) && d.events.length) {
              ...
              localStorage.setItem(AUTO_EXTRACT_KEY, target);
       ...
       * page load we compress the OLDEST day that still has un-compressed
       * input — the WHOLE day's input in one go, dated that day. Once a day
       * is compressed it is marked, so the next open moves to the next one.
       */
      var AUTO_EXTRACT_KEY = "dsh-companion-cat:auto-extract";
      function maybeAutoExtract() {
        try {
          if (!readConfig().deepCompanion) return;
          /* find the OLDEST day (before today) with a bucket that hasn't
             been compressed yet */
          var map = readInputsByDay();
          var done = localStorage.getItem(AUTO_EXTRACT_KEY) || "";
          var candidates = Object.keys(map)
            .filter(function (d) {
              return d < todayStr() && d > done;
            })
            .sort();
          if (!candidates.length) return;
          /* compress the oldest such day once per open */
          setTimeout(function () {
            extractMemoryNow(candidates[0], true);
          }, 12000);
        } catch (_) {}
      }
