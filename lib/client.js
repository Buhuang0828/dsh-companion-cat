...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      "#companion-pet-root .pet-panel-group{font-size:11px;font-weight:700;letter-spacing:.05em;color:#a08040;margin:12px 0 3px;text-transform:uppercase}" +
      ...
      "#companion-pet-root .pet-panel-stats{display:flex;align-items:center;gap:6px;font-size:12px;color:#6a5538;padding:6px 0;flex-wrap:wrap}" +
      "#companion-pet-root .pet-panel-stats b{color:#a08040;font-weight:700}" +
      "#companion-pet-root .pet-panel-stats .ps-last{color:#8a7448;font-size:11px;margin-left:2px}" +
    ...
    function createPet() {
      ...
      function checkAlarms() {
        try {
          ...
          if (cfg.restRemind && !ring.rest) {
            var mins = (Date.now() - pageStart) / 60000;
            if (mins >= (cfg.restInterval || 45)) {
              ring.rest = day;
              say("坐了好一会儿啦，起来伸个懒腰吧~ 🧘");
      ...
      /* ---- daily stats: online time + chat turns (local, zero tokens) ---- */
      var STATS_KEY = "dsh-companion-cat:stats";
      var STATS_LAST_KEY = "dsh-companion-cat:stats-last";
      var lastStatsSave = Date.now();
      /* turn dedupe: DSH re-renders the message list on each reply, rebuilding
         DOM rows — a row whose text was already counted must not count again */
      var seenTurns = new Set();
      function rowFingerprint(n) {
        try {
      ...
      function readStats() {
        try {
          ...
          if (s && s.date) {
            /* v2: turn counting now dedupes; clear counts polluted by the
               re-render bug so the day restarts from a clean number */
            if (s.v !== 2) {
              s.v = 2;
              s.turns = 0;
              s.seeded = false;
        ...
        return { date: todayStr(), seconds: 0, turns: 0, v: 2 };
      }
      ...
      /** Archive yesterday + greet with its summary, once per day (on open). */
      function todayStats() {
        var s = readStats();
        var today = todayStr();
        if (s.date !== today) {
          var y = { date: s.date, seconds: s.seconds, turns: s.turns };
          try {
            localStorage.setItem(STATS_LAST_KEY, JSON.stringify(y));
          } catch (_) {}
          s = { date: today, seconds: 0, turns: 0 };
          writeStats(s);
          if (y.seconds > 0 || y.turns > 0) {
            say("昨天陪你 " + fmtDur(y.seconds) + "，聊了 " + y.turns + " 轮喵~");
          }
        }
        return s;
      }
      ...
      function tickStats() {
        try {
          ...
          if (s.date !== today) {
            try {
              localStorage.setItem(STATS_LAST_KEY, JSON.stringify({ date: s.date, seconds: s.seconds, turns: s.turns }));
            } catch (_) {}
            s = { date: today, seconds: 0, turns: 0 };
          }
          var now = Date.now();
          s.seconds += Math.max(0, Math.floor((now - lastStatsSave) / 1000));
          lastStatsSave = now;
          writeStats(s);
        } catch (_) {}
      }
          ...
          var obs = new MO(function (muts) {
            ...
            for (var i = 0; i < muts.length; i++) {
              ...
              for (var j = 0; j < nodes.length; j++) {
                ...
                if (n.matches && n.matches('[class*="userRow"]')) {
                  var fp = rowFingerprint(n);
                  if (fp && !seenTurns.has(fp)) {
                    seenTurns.add(fp);
                    added++;
                  }
                }
                if (n.querySelectorAll) {
                  var rows = n.querySelectorAll('[class*="userRow"]');
                  for (var k = 0; k < rows.length; k++) {
                    var fp2 = rowFingerprint(rows[k]);
                    if (fp2 && !seenTurns.has(fp2)) {
                      seenTurns.add(fp2);
                      added++;
                    }
                  }
      ...
      /**
       * One-time seed for the day the feature first runs: count the user
       * rows already on screen so today's earlier conversation isn't lost.
       * Runs at most once per day (a `seeded` flag), so refreshes never
       * double-count; fingerprints also enter the dedupe set.
       */
      function seedTurns() {
        try {
          var s = readStats();
          ...
          if (s.seeded) return;
          s.seeded = true;
          var rows = document.querySelectorAll('[class*="userRow"]');
          var n = 0;
          rows.forEach(function (r) {
            var fp = rowFingerprint(r);
            if (fp && !seenTurns.has(fp)) {
              seenTurns.add(fp);
              n++;
            }
      ...
      function openPanel() {
        ...
        panel.innerHTML =
          ...
          '<div class="pet-panel-group">📊 今日统计</div>' +
        ...
        panel.querySelectorAll("[data-key]").forEach(function (el) {
          el.addEventListener("change", function () {
            ...
            if (key === "bg" || key === "bubbles" || key === "lateRemind" || key === "mood" || key === "restRemind") {
              cfg[key] = el.checked;
          ...
          if (last) {
            try {
              ...
              if (y && (y.seconds > 0 || y.turns > 0)) {
                last.textContent = "昨日 " + fmtDur(y.seconds) + " · " + y.turns + " 轮";
              }
            } catch (_) {}
          }
        })();
